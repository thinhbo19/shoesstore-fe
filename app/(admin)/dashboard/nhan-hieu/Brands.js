...
import {
  ...
  postBrand,
  reloadBrand,
} from "@/services/Redux/handle/hanldeBrand";
...
const Brands = ({ brandData }) => {
  ...
  const handleSubmit = async (brandName, setBrandName) => {
    try {
      const res = await postBrand(accessToken, brandName);
      Swal.fire({
        icon: "success",
        text: "Add successfully",
      });
      setBrandName("");
      reloadBrand(setFilteredList);
    } catch (error) {
      Swal.fire({
        icon: "warning",
  ...
  const hanleEdit = async (brandName, setBrandName) => {
    try {
      await putCategory(accessToken, brandName, selected._id);
      Swal.fire({
        icon: "success",
    ...
    <section className="Categories maincontainer section" id="Categories">
      <div className="categories__container ">
        ...
        <div className="pagination">
          {Array.from({
            length: Math.ceil(brandData.length / PerPage),
          }).map((_, index) => (
            <button
              key={index}
